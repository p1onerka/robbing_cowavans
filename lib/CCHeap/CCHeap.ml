...
(* Modifications by Nikita Shchutskii: add and implement Heap's remove_one, remove_bottommost, find *)
...
module type S = sig
  ...
  val remove_one : (elt -> elt -> bool) -> elt -> t -> elt option * t
  (** Delete one occurrence of a value if it exist in the heap.
      [remove_one eq x h], use [eq] to find one [x] in [h] and delete return it with new heap.*)
  
  val remove_bottommost: t -> elt option * t
  ...
  val find : (elt -> elt -> bool) -> elt -> t -> elt option
  val size : t -> int
...
module Make (E : PARTIAL_ORD) : S with type elt = E.t = struct
  ...
  let remove_one eq x h =
    let rec aux = function
    | E -> (None, E)
    | N (_, y, l, r) as h ->
        if eq x y then (Some y, merge l r)
        else if E.leq y x then
          let (el_l, l1) = aux l in
            let (el_r, r1) =
              match el_l with
              | None -> aux r 
              | Some _ -> None, r
            in
              match (el_l, el_r) with
              | (Some el, None) | (None, Some el) -> (Some el, _make_node y l1 r1)
              | _ -> (None, h)
        else (None, h)
    in
      aux h
  
  let rec remove_bottommost = function
    |E -> None, E
    |N (_, y, l, r) -> match remove_bottommost l with
      |None, _ ->  Some y, E
      | Some y1, h -> Some y1,_make_node y h r 
      
  (* let rec remove_bottommost = function
    |E -> None, E
    |N (_, y, l, r) ->
      (match (l, r) with
      | (N (_, y1,_, _), E) | (E, N(_, y1, _,_)) -> (Some y1, _make_node y E E)
      |_ -> (let deeper_subtree, another_subtree = match l, r with
        | (N (lrank, _, _, _), N (rrank, _,_ ,_)) when lrank > rrank -> l, r
        | _ -> r, l in
          match remove_bottommost deeper_subtree with
          | None, _ -> (Some y, E)
          | Some y1, h -> (Some y1, _make_node y another_subtree h))) *)
